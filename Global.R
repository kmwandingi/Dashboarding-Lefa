...
# sep19 <- read_excel("20. Sep 2019 Orders.xlsx")
# oct19 <- read_excel("21. Oct 2019 Orders.xlsx")
# nov19 <- read_excel("22. Nov 2019 Orders.xlsx")
# dec19 <- read_excel("23. Dec 2019 Orders.xlsx")
# jan20 <- read_excel("24. Jan 2020 Orders.xlsx")
# feb20 <- read_excel("25. Feb 2020 Orders.xlsx")
# #check colnames that are not in all dfs
# #setdiff(names(nov18), names(may18))
# #--------------
# O18 <- rbind.fill(feb18, apr18, mar18, may18, jun18, jul18, aug18, sep18, oct18, nov18, dec18, jan19, feb19, mar19, apr19, may19, jun19, jul19, aug19, sep19, oct19, nov19, dec19, jan20, feb20)
# saveRDS(O18, "O18_1.rds")
# 
# lapply(sep19, class)
# lapply(aug19, class)
# 
# rm(feb18, apr18, mar18, may18, jun18, jul18, aug18, sep18, oct18, nov18, dec18,
#    jan19, feb19, mar19, apr19, may19, jun19, jul19, aug19, oct19, nov19, jan20, feb20)

O18 <- readRDS(file = "O18_1.rds")
...
#Some orders have missing order ids, lat, long etc...
# da <- O18 %>% dplyr::select(long, lat,'order id', 'order source') %>% dplyr::rename(Longitude = long ,Latitude = lat)
# d <- da[is.na(.),]
# d <-
#   da %>%
...
condat <- condat%>% dplyr::rename(phone = 'passenger phone', email = 'passenger email', passenger='passenger name', cost = "trip cost, NAD", ratingbydriver = "rating by driver", 
                                  ratingbypass = "rating by passenger", driver ="driver name")
...
condat$MonYear <- as.yearmon(condat$ymd)

wr <- as.data.frame(condat %>% 
                      dplyr::group_by(passenger, phone, email, ratingbypass) %>%
                      dplyr::summarise(Rating = mean(ratingbypass)))

wri <- subset(wr, Rating == min(wr$ratingbypass, na.rm = TRUE))

HTML(paste0(wri$passenger, "<br>", wri$phone,"<br>", wri$email, "<br><br>Orders: ", wri$Rating))


#summarise orders

dayHour <- ddply(condat, c( "hour", "wday", 'Constituency', 'MonYear'), summarise,
...
colPal <- colorRampPalette(c('brown','red','orange', 'blue', 'green', 'purple', 'pink'))(length(x_unique))
...
chord <- chord%>%dplyr::select(Constituency, "order status.x", "cancellation reason.x", bundle.x, MonYear)  %>% dplyr::rename(bundle=bundle.x, order.status = "order status.x", cancellation.reason = "cancellation reason.x")
